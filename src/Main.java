...
public class Main {
			...
			if ((usuarioLogado.getSenha().equals(senha))) {
				...
				if (usuarioLogado.IsEmpresa()) {
					...
					case 1: {
						...
						System.out.println("Saldo Empresa: " + (usuarioLogado.getEmpresa().getSaldo() - usuarioLogado.getEmpresa().getSaldo()*usuarioLogado.getEmpresa().getTaxa()));
						System.out.println("************************************************************");
					...
					case 1: {
						...
						if(!isInputValid(escolhaEmpresa, empresas)){
							System.out.println("Empresa invalida tente novamente ");
							executar(usuarios, clientes, empresas, produtos, carrinho, vendas);
						}
	...
	public static boolean isInputValid(Integer id, List<Empresa>list){		
		 return list.stream().filter(p -> p.getId().equals(id)).findAny().isPresent();
	}
}
import java.util.ArrayList;
import java.util.Arrays;
